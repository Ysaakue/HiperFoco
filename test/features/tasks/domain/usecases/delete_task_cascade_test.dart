...
/// taskId foreign keys on Reminders/TaskOccurrenceOverrides (Drift doesn't
/// turn on `PRAGMA foreign_keys` by default), so nothing would catch this
/// at the schema level if DeleteTask stopped cleaning up after itself.
void main() {
  test('deleting a task also removes its reminder and occurrence overrides',
      () async {
    ...
    await DeleteTask(taskRepository, reminderRepository, overrideRepository)(taskId);
